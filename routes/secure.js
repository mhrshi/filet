...
secure.post('/restricted/resetter', async (req, res) => {
	jwt.verify(req.cookies.FiletLog, process.env.LOL, (_, data) => {
...
secure.post('/revise', async (req, res) => {
	...
	database
		.result(`UPDATE ${req.body.subject} SET fileid='${req.body.fileid}' WHERE e_no='${req.body.username}' AND id=${req.body.pracid}`)
		.then(result => {
...
secure.post('/practicals/deadline', async (req, res) => {
	...
	database
		.result(`UPDATE ${req.body.subject}_pracs
